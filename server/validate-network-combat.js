...
const WS_PORT = 8191;
const ROOM_ID = "validate-network-combat";
...
async function captureFailure(page, error, state = null) {
  mkdirSync(artifactsDir, { recursive: true });
  const screenshotPath = resolve(artifactsDir, "validate-network-combat-failure.png");
  const statePath = resolve(artifactsDir, "validate-network-combat-failure.json");
  try {
...
async function main() {
  ...
  try {
    ...
    await page.locator("#net-player-name-setup").fill("CombatValidator");
    ...
    assert(lastState.player.classType === "fighter", `expected fighter/warrior class, got ${lastState.player.classType}`);
    assert(lastState.walkable === true, `player joined in blocked space: ${JSON.stringify(lastState)}`);

    const startX = lastState.player.x;
    await page.keyboard.down("d");
    await delay(450);
    await page.keyboard.up("d");
    await delay(250);
    const movedState = await page.evaluate(() => window.__WOTC_DEBUG__?.getState?.() || null);
    assert(movedState, "debug state unavailable after movement");
    assert(movedState.player.x > startX + 4, `fighter did not move right after join: start=${startX}, end=${movedState.player.x}`);
    assert(movedState.walkable === true, `fighter entered blocked space after movement: ${JSON.stringify(movedState)}`);

    const canvas = page.locator("#game");
    const box = await canvas.boundingBox();
    assert(box, "game canvas bounding box unavailable");
    const preAttackState = movedState;
    await page.mouse.move(box.x + box.width * 0.55, box.y + box.height * 0.45);
    await page.mouse.click(box.x + box.width * 0.55, box.y + box.height * 0.45, { button: "left" });

    await page.waitForFunction((beforeCount) => {
      const state = window.__WOTC_DEBUG__?.getState?.();
      return !!state && state.combat && state.combat.meleeSwingCount > beforeCount;
    }, preAttackState.combat.meleeSwingCount, { timeout: 2500 });

    const attackState = await page.evaluate(() => window.__WOTC_DEBUG__?.getState?.() || null);
    assert(attackState, "debug state unavailable after attack");
    assert(
      attackState.combat.meleeSwingCount > preAttackState.combat.meleeSwingCount,
      `fighter attack did not create melee swing output: before=${preAttackState.combat.meleeSwingCount}, after=${attackState.combat.meleeSwingCount}`
    );

    mkdirSync(artifactsDir, { recursive: true });
    const successPath = resolve(artifactsDir, "validate-network-combat-success.json");
    writeFileSync(successPath, JSON.stringify({
      joined: lastState,
      moved: movedState,
      attacked: attackState
    }, null, 2));
    console.log(JSON.stringify({
      joinedWalkable: lastState.walkable,
      movedDx: movedState.player.x - startX,
      meleeSwingsBefore: preAttackState.combat.meleeSwingCount,
      meleeSwingsAfter: attackState.combat.meleeSwingCount,
      successPath
    }, null, 2));
  } catch (error) {
