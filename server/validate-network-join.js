...
const WS_PORT = 8190;
const ROOM_ID = "validate-network-join";
...
async function captureFailure(page, error, state = null) {
  ...
  const screenshotPath = resolve(artifactsDir, "validate-network-join-failure.png");
  const statePath = resolve(artifactsDir, "validate-network-join-failure.json");
  try {
...
async function main() {
  ...
  try {
    ...
    assert(lastState.tile?.value !== "#", `player joined on wall tile: ${JSON.stringify(lastState.tile)}`);
    ...
    assert(movedState.walkable === true, `player entered blocked space after movement: ${JSON.stringify(movedState)}`);
    assert(movedState.player.x > startX + 4, `player did not move right after join: start=${startX}, end=${movedState.player.x}`);

    mkdirSync(artifactsDir, { recursive: true });
    const successPath = resolve(artifactsDir, "validate-network-join-success.json");
    writeFileSync(successPath, JSON.stringify({
      joined: lastState,
      moved: movedState
    }, null, 2));
    console.log(JSON.stringify({
      joinedTile: lastState.tile,
